...
def clean_content(
    ...
    if not is_valid:
        ...
        # embed media
        context.prob('error', 'Invalid content pushed to media embed - content type: {}'.format(from_type_passed), u'Page had invalid XML after cleaning:{error_log}\ncontent:\n{content}\ncontent_type:{}'.format(content=content, error_log=schema.error_log, from_type_passed))
        context = context._replace(slots=[])
        doc = _to_etree(context, content, import_as)
